...
public class TreeEditDistance<T> {
	...
	public int treeEditDistance(TreeNode<NumberedObjectContainer<T>> T1, TreeNode<NumberedObjectContainer<T>> T2) {
		...
		int td[][] = new int[TreeUtil.calculateSize(T1)][TreeUtil.calculateSize(T2)];
		int[] l1 = lmld(T1, new int[TreeUtil.calculateSize(T1)]); int[] kr1 = kr(l1 , TreeUtil.calculateNumberOfLeaves(T1));
		int[] l2 = lmld(T2, new int[TreeUtil.calculateSize(T2)]); int[] kr2 = kr(l2 , TreeUtil.calculateNumberOfLeaves(T2));
		for (int x = 1;  x <= kr1.length; x++) {
		    for (int y = 1; y <= kr2.length; y++) {
		        forestDist(kr1[x], kr2[y ], l1 , l2 , td);
		    }
		}
		return td[td.length][td[td.length].length];
	}
	...
	private void forestDist(int i, int j, int[] l1, int[] l2, int[][] td) {
		 Displaced2DIntegerArray fd = new Displaced2DIntegerArray(l1[i] - 1, i, l2 [j] - 1, j);
		 ...
		 for (int di = l1[i]; di <= i; di++)
		     for (int dj = l2[j]; dj <= j; dj++)
		            if (l1[di] == l1[i] && l2[dj] == l2[j]) {	//if (l[di] == l[i] && l[dj] == l[j]) {
		               fd.set(di, dj, min(fd.get(di - 1, dj) + W_DEL,
		                                     fd.get(di, dj - 1) + W_INS,
		                                     fd.get(di - 1, dj - 1) + W_REN));
		               td[di][dj] = fd.get(di, dj);	//td.set(di , dj, f[di, dj];
		            } else {
		            	fd.set(di, dj, min(fd.get(di - 1, dj) + W_DEL,
		                                         fd.get(di , dj - 1) + W_INS,
		                                         fd.get(l1[di] - 1, l2[dj] - 1) + td[di][dj]));	//fd.get(l[di] - 1, l[dj] - 1) + td[di][dj]));
		            }
